...
def evaluate_config(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        object.__setattr__(
            cfg_module.config, "quality_threshold", original_p["quality_threshold"]
        )
        object.__setattr__(
            cfg_module.config, "max_market_ratio", original_p["max_market_ratio"]
        )
        object.__setattr__(
            cfg_module.config, "target_win_rate", original_p["target_win_rate"]
        )
...
def run_grid_search():
    ...
    logger.info(
        "Starting Hyperparameter Optimization with {} candidates...".format(
            len(combinations)
        )
    )
    ...
    if not valid_results:
        logger.warning(
            "No config met Budget Utilization >= 85%. Falling back to absolute best ROI."
        )
