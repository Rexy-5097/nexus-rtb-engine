...
from scipy.sparse import csr_matrix, vstack  # noqa: E402
...
def task1_reduce_overfitting(X, y_ctr, y_cvr):
    ...
    for fold in range(5):
        ...
        logger.info(
            f"  Fold {fold+1}: Train AUC={auc_tr:.4f}, Val AUC={auc_va:.4f}, Gap={auc_tr - auc_va:.4f}"
        )
    ...
    logger.info(
        f"  K-Fold Mean: Train={mean_tr:.4f}, Val={mean_val:.4f}, Gap={mean_gap:.4f}"
    )
    ...
    logger.info(
        f"  Feature Pruning: Keeping {n_kept}/{n_total} features (dropped {n_total - n_kept})"
    )
    ...
    logger.info(
        f"  FINAL CTR: Train={auc_tr_final:.4f}, Test={auc_te_final:.4f}, Gap={gap_final:.4f}"
    )

    ctr_metrics = {
        "train_auc": auc_tr_final,
        "test_auc": auc_te_final,
        "gap": gap_final,
        "kfold_mean": mean_val,
    }
    ...
    if X_tr_cvr.shape[0] > 50 and y_tr_cvr.sum() > 5:
        ...
        final_cvr = train_lgbm(
            X_tr_cvr, y_tr_cvr, X_te_cvr, y_te_cvr, base_params, spw_cvr
        )
        ...
        logger.info(
            f"  FINAL CVR: Train={cvr_tr:.4f}, Test={cvr_te:.4f}, Gap={cvr_tr - cvr_te:.4f}"
        )
...
def task2_regularization_sweep(X, y_ctr):
    ...
    for i, r in enumerate(results[:5]):
        logger.info(
            f"    {i+1}. L1={r['l1']}, L2={r['l2']}, FF={r['ff']}, BF={r['bf']} → "
...
def task3_calibration_stability(model, X, y_ctr, prices):
    ...
    logger.info(
        f"  Baseline: AUC={base_auc:.4f}, ECE={base_ece:.4f}, Brier={base_brier:.4f}"
    )
    ...
    logger.info(
        f"  CTR Spike: AUC={spike_auc:.4f}, ECE={spike_ece:.4f}, Brier={spike_brier:.4f}"
    )
    ...
    logger.info(
        f"  Price Shift (noise): AUC={shift_auc:.4f}, ECE={shift_ece:.4f}, Brier={shift_brier:.4f}"
    )
    ...
    logger.info(
        f"  Data Drift: AUC={drift_auc:.4f}, ECE={drift_ece:.4f}, Brier={drift_brier:.4f}"
    )
...
def task4_budget_efficiency(ctr_model, cvr_model, X, y_ctr, y_cvr, prices):
    ...
    logger.info(
        f"  Constant:   Wins={const_wins}, Clicks={const_clicks}, Convs={const_convs}, ROI={const_roi:.4f}, BudgetUtil={const_spend/BUDGET:.4f}"
    )
    logger.info(
        f"  Static EV:  Wins={static_wins}, Clicks={static_clicks}, Convs={static_convs}, ROI={static_roi:.4f}, BudgetUtil={static_spend/BUDGET:.4f}"
    )
    logger.info(
        f"  Dynamic EV: Wins={dyn_wins}, Clicks={dyn_clicks}, Convs={dyn_convs}, ROI={dyn_roi:.4f}, BudgetUtil={dyn_spend/BUDGET:.4f}"
    )
    ...
    return {
        "constant": {
            ...
            "clicks": const_clicks,
            "convs": const_convs,
            "budget_util": const_spend / BUDGET,
        },
        "static": {
            "roi": static_roi,
            "wins": static_wins,
            "clicks": static_clicks,
            "convs": static_convs,
            "budget_util": static_spend / BUDGET,
        },
        "dynamic": {
            "roi": dyn_roi,
            "wins": dyn_wins,
            "clicks": dyn_clicks,
            "convs": dyn_convs,
            "budget_util": dyn_spend / BUDGET,
            "final_multiplier": bid_multiplier,
        },
    }
...
def task5_cvr_penalization(cvr_model, X, y_ctr, y_cvr, stats):
    ...
    return {
        "raw_auc": base_auc,
        "penalized_auc": adj_auc,
        "auc_change": adj_auc - base_auc,
        "std_dev": std_dev,
    }
...
def task6_stress_testing(ctr_model, cvr_model, X, y_ctr, y_cvr, prices):
    ...
    results["unseen_domains"] = {
        "auc": auc_a,
        "auc_drop": base_auc - auc_a,
        "roi": roi_a,
    }
...
def task7_final_report(
    ...
    model_size = (
        os.path.getsize(model_path) / (1024 * 1024) if os.path.exists(model_path) else 0
    )
    ...
    logger.info(f"{'='*70}")
    logger.info(
        f"{'CTR AUC (Train)':<35} {ctr_metrics.get('train_auc',0):<15.4f} {'':<15}"
    )
    logger.info(
        f"{'CTR AUC (Test)':<35} {ctr_metrics.get('test_auc',0):<15.4f} {'≥ 0.62':<15} {'✅' if ctr_metrics.get('test_auc',0) >= 0.62 else '❌'}"
    )
    logger.info(
        f"{'CTR Gap':<35} {ctr_gap:<15.4f} {'< 0.03':<15} {'✅' if ctr_gap < 0.03 else '⚠️'}"
    )
    logger.info(
        f"{'CVR AUC (Train)':<35} {cvr_metrics.get('train_auc',0):<15.4f} {'':<15}"
    )
    logger.info(
        f"{'CVR AUC (Test)':<35} {cvr_metrics.get('test_auc',0):<15.4f} {'≥ 0.58':<15} {'✅' if cvr_metrics.get('test_auc',0) >= 0.58 else '❌'}"
    )
    logger.info(
        f"{'CVR Gap':<35} {cvr_gap:<15.4f} {'< 0.03':<15} {'✅' if cvr_gap < 0.03 else '⚠️'}"
    )
    logger.info(f"{'-'*70}")
    logger.info(
        f"{'ROI (Dynamic Bidding)':<35} {dyn.get('roi',0):<15.4f} {'≥ 0.85':<15} {'✅' if dyn.get('roi',0) >= 0.85 else '❌'}"
    )
    logger.info(f"{'ROI (Static EV)':<35} {static.get('roi',0):<15.4f} {'':<15}")
    logger.info(f"{'ROI (Constant Bid)':<35} {const.get('roi',0):<15.4f} {'':<15}")
    roi_lift = (
        (dyn.get("roi", 0) - const.get("roi", 0.001))
        / max(const.get("roi", 0.001), 0.001)
    ) * 100
    logger.info(f"{'ROI Lift vs Constant':<35} {f'{roi_lift:+.1f}%':<15} {'':<15}")
    logger.info(
        f"{'Budget Utilization':<35} {dyn.get('budget_util',0):<15.4f} {'':<15}"
    )
    ...
    logger.info(f"{'-'*70}")
    logger.info(
        f"{'Latency (ms/sample)':<35} {latency:<15.4f} {'< 5ms':<15} {'✅' if latency < 5 else '❌'}"
    )
    logger.info(
        f"{'Model Size (MB)':<35} {model_size:<15.2f} {'< 50MB':<15} {'✅' if model_size < 50 else '❌'}"
    )
    logger.info(f"{'='*70}")
    ...
    logger.info(
        f"{'Max AUC Drop under stress':<35} {stress_results.get('max_auc_drop',0):<15.4f} {'< 0.05':<15} {'✅' if stress_results.get('max_auc_drop',0) < 0.05 else '⚠️'}"
    )
    logger.info(
        f"{'All ROI Positive under stress':<35} {'YES' if stress_results.get('all_roi_positive',False) else 'NO':<15} {'YES':<15} {'✅' if stress_results.get('all_roi_positive',False) else '❌'}"
    )
    ...
    # Readiness
    all_pass = (
        ctr_metrics.get("test_auc", 0) >= 0.62
        and ctr_gap < 0.03
        and dyn.get("roi", 0) >= 0.85
        and latency < 5
        and model_size < 50
        and stress_results.get("all_roi_positive", False)
    )

    mostly_pass = (
        ctr_metrics.get("test_auc", 0) >= 0.60
        and ctr_gap < 0.05
        and dyn.get("roi", 0) >= 0.5
        and latency < 5
        and model_size < 50
    )
    ...
    if all_pass:
        ...
        if ctr_gap >= 0.03:
            logger.info(
                f"   ⚠️ CTR gap ({ctr_gap:.4f}) slightly above 0.03 — monitor for drift."
            )
        if dyn.get("roi", 0) < 0.85:
            logger.info(
                f"   ⚠️ ROI ({dyn.get('roi',0):.4f}) below 0.85 — tune bid multiplier."
            )
    else:
...
def main():
    ...
    # Task 1
    ctr_model, cvr_model, ctr_metrics, cvr_metrics = task1_reduce_overfitting(
        X, y_ctr, y_cvr
    )
    ...
    # Task 4
    budget_results = task4_budget_efficiency(
        ctr_model, cvr_model, X, y_ctr, y_cvr, prices
    )
