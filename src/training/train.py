...
def match_type(name: str):
    if fnmatch.fnmatch(name, "imp.*.txt"): return "IMP"
    if fnmatch.fnmatch(name, "clk.*.txt"): return "CLK"
    if fnmatch.fnmatch(name, "conv.*.txt"): return "CONV"
    return None
    ...
    with open(os.path.join(root, "conv.20131019.txt"), "w") as f:
        for imp in impression_meta:
            if imp["bidid"] in clicked_bids:
                ...
                if imp["floor"] > 20: conv_rate *= 1.5  # Higher floor = better quality
...
def discover_items(root: str):
    ...
    for dirpath, _, filenames in os.walk(root):
        for fname in filenames:
                ...
                try:
                    with zipfile.ZipFile(fpath) as zf:
                        for info in zf.infolist():
                            if info.is_dir() or info.file_size > 2e9: continue
                            typ = match_type(os.path.basename(info.filename))
                            if typ:
                                items.append({
                                    "type": typ, "kind": "zip", "zip_path": fpath, 
                                    "entry": info.filename, "name": info.filename,
            ...
            if typ:
                items.append({
                    "type": typ, "kind": "file", "path": fpath, 
                    "name": fname, "display": fpath
                })
    return items
...
def index_events(conn, items, table):
    ...
    for item in items:
        for chunk in iter_chunks(item, usecols=[0]):
            ...
            if bidids:
                cur.executemany(f"INSERT OR IGNORE INTO {table} (bidid) VALUES (?)", [(b,) for b in bidids])
        conn.commit()
...
def compute_rolling_stats(df, stats):
    ...
    recs = df[['timestamp', 'advertiser', 'domain', 'fmt', 'vis', 'click']].to_dict('records')
...
def load_dataset(data_root="data"):
    ...
    if not imps: sys.exit("No data found even after generation attempt")
    ...
    full_df['payingprice'] = pd.to_numeric(full_df['payingprice'], errors='coerce').fillna(0)
    full_df = full_df[full_df['payingprice'] > 0]
    
    # 1b. Floor Check (New for Phase 7)
    if 'floorprice' in full_df.columns:
        full_df['floorprice'] = pd.to_numeric(full_df['floorprice'], errors='coerce').fillna(0)
    ...
    logger.info(f"Phase 7: Data Filtered. New Size: {len(full_df)} (Dropped {initial_len - len(full_df)})")
    ...
    logger.info(f"Top-K Maps: { {k: len(v) for k,v in top_k_maps.items()} }")
    ...
    if 'userid' in full_df.columns:
        ...
        full_df['cum_user_imps'] = full_df.groupby('userid').cumcount()  # 0-indexed count before this row
        full_df['cum_user_clicks'] = full_df.groupby('userid')['click'].cumsum() - full_df['click']  # exclude current
        
        # user_ctr = prior_clicks / prior_imps (with smoothing)
        full_df['user_ctr'] = (full_df['cum_user_clicks'] + 1) / (full_df['cum_user_imps'] + 20)
        full_df['user_count_7d'] = full_df['cum_user_imps']  # Approximation (all history as proxy for 7d)
    ...
    num_stats = {k: {"sum":0.0, "sq":0.0, "n":0} for k in ["region", "city", "adslot_visibility", "adslot_format", "ad_slot_area"]}
    global_stats = {} 
    ...
    def update_counter(k, price, click, conv):
        if k not in global_stats: global_stats[k] = [0, 0.0, 0, 0]
        s = global_stats[k]
        s[0] += 1
        s[1] += price
        if click: s[2] += 1
    ...
    full_df['conversion'] = pd.to_numeric(full_df['conversion'], errors='coerce').fillna(0)

    # 1. Global Aggregates (Adv, Dom, Adv-Dom)
    # Advertiser
    g = full_df.groupby('advertiser').agg({'payingprice': ['count', 'sum'], 'click': 'sum', 'conversion': 'sum'})
    for adv, row in g.iterrows():
        global_stats[str(adv)] = [row[('payingprice','count')], row[('payingprice','sum')], row[('click','sum')], row[('conversion','sum')]]
        
    # Domain
    g = full_df.groupby('domain').agg({'payingprice': ['count', 'sum'], 'click': 'sum', 'conversion': 'sum'})
    for dom, row in g.iterrows():
        global_stats[f"domain:{dom}"] = [row[('payingprice','count')], row[('payingprice','sum')], row[('click','sum')], row[('conversion','sum')]]
        
    # Adv-Domain
    # NOTE: Groupby multiple columns
    g = full_df.groupby(['advertiser', 'domain']).agg({'payingprice': ['count', 'sum'], 'click': 'sum', 'conversion': 'sum'})
    for (adv, dom), row in g.iterrows():
        global_stats[f"adv_domain:{adv}_{dom}"] = [row[('payingprice','count')], row[('payingprice','sum')], row[('click','sum')], row[('conversion','sum')]]
    ...
    for col in ["region", "city", "vis", "fmt"]:
        ...
        target_k = col
        if col == "vis": target_k = "adslot_visibility"
        if col == "fmt": target_k = "adslot_format"
...
def main():
    ...
    if not X_parts: sys.exit("No training data loaded")
    
    X_full = vstack(X_parts)
    y_ctr_full = np.concatenate(y_ctr_parts)
    y_cvr_full = np.concatenate(y_cvr_parts)
    
    logger.info(f"Loaded Dataset: {X_full.shape} samples. CTR+: {y_ctr_full.mean():.4f} CVR+: {y_cvr_full.mean():.4f}")
    ...
    logger.info(f"CVR Train Samples: {X_tr_cvr.shape[0] if X_tr_cvr is not None else 0}")
    ...
    def train_and_select(X_t, y_t, X_v, y_v, X_tt, y_tt, label="CTR"):
        ...
        if pos_rate < 0.2:
            ...
            logger.info(f"Applying Class Weighting (Pos Rate={pos_rate:.4f}, LGB Scale={scale_pos_weight:.2f})")
        ...
        if lgb and X_t.shape[0] > 500:
            lgb_configs = [
                {"n_estimators": 300, "learning_rate": 0.03, "num_leaves": 15, "max_depth": 4, "min_data_in_leaf": 200, "feature_fraction": 0.7, "bagging_fraction": 0.8, "bagging_freq": 5, "lambda_l1": 10.0, "lambda_l2": 10.0},
                {"n_estimators": 500, "learning_rate": 0.02, "num_leaves": 15, "max_depth": 4, "min_data_in_leaf": 300, "feature_fraction": 0.5, "bagging_fraction": 0.6, "bagging_freq": 5, "lambda_l1": 5.0, "lambda_l2": 5.0},
            ]
            for lgb_cfg in lgb_configs:
                try:
                    ...
                    probs = model.predict_proba(X_v)[:, 1]
                    auc = roc_auc_score(y_v, probs)
                    logger.info(f"LGBM({lgb_cfg['num_leaves']}L, d{lgb_cfg['max_depth']}) Val AUC: {auc:.4f}")
                    if auc > best_score:
                        best_score = auc
                        best_mdl = model
        ...
        if best_mdl:
            ...
            try: auc_tr = roc_auc_score(y_t, probs_tr)
            except: auc_tr = 0.5
            ...
            logger.info(f"{label} TRAIN AUC={auc_tr:.4f}, TEST AUC={auc_te:.4f}, Gap={auc_gap:.4f} {overfit_flag}")
            logger.info(f"{label} TEST: LogLoss={ll:.4f}, Brier={br:.4f}, ECE={ece:.4f}")
    ...
    final_ctr_model, ctr_params, ctr_calib = train_and_select(X_tr, y_ctr_tr, X_va, y_ctr_va, X_te, y_ctr_te, "CTR")
    final_cvr_model, cvr_params, cvr_calib = train_and_select(X_tr_cvr, y_tr_cvr, X_va_cvr, y_va_cvr, X_te_cvr, y_te_cvr, "CVR")
    ...
    logger.info(f"TRAIN STATS: Imps={total_imps} Spend={total_spend:.2f} Clicks={total_clicks} Convs={total_convs}")
