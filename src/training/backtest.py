...
def run_backtest():
    ...
    # 5. AUC
    try:
        ctr_auc = roc_auc_score(y_click_test, pCTR)
    except:
        ctr_auc = 0.5

    try:
        cvr_auc = roc_auc_score(y_conv_test, pConv)
    except:
        cvr_auc = 0.5
    ...
    logger.info(
        f"{'Budget Utilization':<25} {budget_util:<15.4f} {const_spend/BUDGET if BUDGET > 0 else 0:<15.4f}"
    )
    ...
    # Lift
    click_lift = (
        ((total_clicks - const_clicks) / const_clicks * 100) if const_clicks > 0 else 0
    )
    conv_lift = (
        ((total_convs - const_convs) / const_convs * 100) if const_convs > 0 else 0
    )
    ...
    logger.info(
        f"CTR AUC >= 0.62: {'✅ PASS' if ctr_auc >= 0.62 else '❌ FAIL'} ({ctr_auc:.4f})"
    )
    logger.info(
        f"CVR AUC >= 0.58: {'✅ PASS' if cvr_auc >= 0.58 else '❌ FAIL'} ({cvr_auc:.4f})"
    )
    logger.info(
        f"Latency < 5ms:   {'✅ PASS' if latency_ctr < 5 else '❌ FAIL'} ({latency_ctr:.4f}ms)"
    )
    logger.info(
        f"Model < 512MB:   {'✅ PASS' if model_size_mb < 512 else '❌ FAIL'} ({model_size_mb:.2f}MB)"
    )
