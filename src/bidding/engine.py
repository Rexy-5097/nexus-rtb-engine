...
class BiddingEngine:
    ...
    def process(self, request: BidRequest) -> BidResponse:
        ...
        try:
            ...
            # Fallback / Safety
            if p_ctr_model < 0 or p_ctr_model > 1:
                p_ctr_model = 0.001
            if p_cvr < 0 or p_cvr > 1:
                p_cvr = 0.0
            ...
            if adv_count < 100:  # Low-impression advertiser
                ...
                ev = (p_ctr * config.value_click) + (
                    p_conv_imp_adj * config.value_conversion
                )
            ...
            if ev < pred_mp * 0.8:
                return BidResponse(
                    ...
                    bidPrice=0,
                    advertiserId=adv_id,
                    explanation=f"unprofitable_ev={ev:.2f}_mp={pred_mp:.2f}",
                )
            ...
            self._roi_window.append(
                (final_bid * target_wr, 0)
            )  # Value updated on feedback
            ...
            return BidResponse(
                ...
                bidPrice=final_bid,
                advertiserId=adv_id,
                explanation=f"ok_lat={latency_ms:.3f}ms",
            )
